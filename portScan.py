...
def conn(minport, maxport, timeout, protocol, returnType, resultStore, retries):
    if(protocol == 1):  #TCP
...
if(scanm == 2):
    ...
    for i in range (0, threadCount):
        if(i == threadCount-1):   #because rounding down with portsPerThr >> to catch excess ports
            ...
            t = threading.Thread(target=conn, args=(minport + i*portsPerThr, maxport, timeout/1000, protocol, returnType, resultStore, retries))
            threads.append(t)
        else:
            print("Thread", i + 1, "has ports", minport + i * portsPerThr, "through", minport + (i+1)*portsPerThr - 1, "\n")
            t = threading.Thread(target=conn, args=(minport + i*portsPerThr, minport + (i+1)*portsPerThr - 1, timeout/1000, protocol, returnType, resultStore, retries))
            threads.append(t)
    for t in threads:
        t.start()
    for t in threads:
...
if(scanm == 1):
    #sequential
    start = time.time()
    conn(minport, maxport, timeout/1000, protocol, returnType, resultStore, retries)
    print("Scan completed in", time.time() - start, "seconds\n")
